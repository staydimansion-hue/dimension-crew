...
/**
 * 운영방 채널의 최근 메시지를 가져옵니다. (conversations.history)
 * 봇 토큰에 읽기 스코프(channels:history / groups:history)가 필요합니다.
 */
export async function fetchChannelHistory(limit = 50): Promise<SlackMessage[]> {
  const token = process.env.SLACK_BOT_TOKEN;
  const channel = process.env.SLACK_OPERATIONS_CHANNEL_ID;

  if (!token || !channel) {
    throw new Error(
      "SLACK_BOT_TOKEN / SLACK_OPERATIONS_CHANNEL_ID 환경변수가 설정되지 않았습니다."
    );
  }
